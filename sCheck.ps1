...
function Write-CustomOut ($Details){
	...
	#write-eventlog -logname Application -source "Windows Error Reporting" -eventID 12345 -entrytype Information -message "sCheck: $Details"
}
...
$Plugins = Get-ChildItem -Path $PluginsFolder -filter "*.ps1" -Recurse | Sort Directory, Name
...
If ($SetupSetting) {
	...
	Write-Host -ForegroundColor Yellow "To define a path to store each sCheck report please use sCheck.ps1 -Outputpath C:\tmp"
	Write-Host 
...
# Adding all plugins
$TTRReport = @()
$MyReport = Get-CustomHTML "$Server sCheck"
...
if ($DisplayToScreen -or $SetupSetting) {
	...
	$Filename = $Env:TEMP + "\" + $Server + "sCheck" + "_" + $Date.Day + "-" + $Date.Month + "-" + $Date.Year + ".htm"
...
if ($Outputpath) {
	...
	$Filename = $ArchiveFilePath + "\" + $VIServer + "_sCheck_" + $DateHTML + ".htm"
	$MyReport | out-file -encoding ASCII -filepath $Filename
}
...
if ($SendEmail) {
	...
	If ($SendAttachment) {
		send-Mailmessage -To $EmailTo -From $EmailFrom -Subject $EmailSubject -SmtpServer $SMTPSRV -Body "sCheck attached to this email" -Attachments $Filename
	} Else {
